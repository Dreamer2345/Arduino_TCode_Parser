...
/**
 * @brief Main TCode Class Holding TCodeAxis and TCodeBuffer
 */
...
public:
    ...
    bool axisRegister(const TCode_ChannelID &id, String Name);                                                  // Function to name and activate axis
    ...
    bool axisRegister(const TCode_Channel_Type &type, const int channel_int, String Name);
...
// Repeated Axis Functions depriciated but still here for backwards compatability if TCODE_USE_STRING_IDS is defined
    ...
    bool axisRegister(const String &id, String Name);
...
template <unsigned TCODE_CHANNEL_COUNT>
bool TCode<TCODE_CHANNEL_COUNT>::axisRegister(const TCode_ChannelID &inputID, String axisName) // register an axis with a string name
{
    TCodeAxis *axis = nullptr;
    if (tryGetAxis(inputID, axis))
    {
...
template <unsigned TCODE_CHANNEL_COUNT>
bool TCode<TCODE_CHANNEL_COUNT>::axisRegister(const TCode_Channel_Type &type, const int channel_int, String Name)
{
    return axisRegister(TCodeParser::constructID(type, channel_int), Name);
}
...
template <unsigned TCODE_CHANNEL_COUNT>
bool TCode<TCODE_CHANNEL_COUNT>::axisRegister(const String &id, String Name)
{
    return axisRegister(TCodeParser::getIDFromStr(id, 0), Name);
}
...
{
    int memloc = -1;
    if (TCodeParser::idValidRange(id,TCODE_CHANNEL_COUNT))
    {
        int typeoffset = -1;
        switch (id.type)
        {
...
void TCode<TCODE_CHANNEL_COUNT>::runAxisCommand(const TCode_Axis_Command &command)
{
    ...
    if (!TCodeParser::idValidRange(command.ID,TCODE_CHANNEL_COUNT))
        return;
...
bool TCode<TCODE_CHANNEL_COUNT>::tryGetAxis(const TCode_ChannelID &id, TCodeAxis *&axis) // Trys to get the axis for the ID given
{
    ...
    if (!TCodeParser::idValidRange(id,TCODE_CHANNEL_COUNT)) // Check if the ID is valid
        return false;
